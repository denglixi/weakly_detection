...
def multi_resolution_feature_maps(feature_map_layout,
    ...
    for index, from_layer in enumerate(feature_map_layout['from_layer']):
        ...
        if from_layer:
            ...
            if 'layer_padding' in feature_map_layout:
                padding = feature_map_layout['layer_padding'][index]
            else:
                padding = 'SAME'
            if 'layer_stride' in feature_map_layout:
                stride = feature_map_layout['layer_stride'][index]
            else:
                stride = 2
            ...
            if insert_1x1_conv:
                layer_name = '{}_1_Conv2d_{}_1x1_{}'.format(
                    base_from_layer, index, layer_depth / 2)
            ...
            if use_depthwise:
                ...
                feature_map = slim.conv2d(
                    intermediate_layer,
                    layer_depth, [conv_kernel_size, conv_kernel_size],
                    padding=padding,
                    stride=stride,
                    scope=layer_name)
