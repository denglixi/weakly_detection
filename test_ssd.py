...
has_sigmoid = False
if has_sigmoid:
    save_path = "./results/softmax_split_sigmoid/model.ckpt-190000"
else:
    save_path = "./results/log-softmax_loss-split_layer/model.ckpt-260000"
...
for key,feature in feature_maps.items():
    ...
    if has_sigmoid:
        feature_maps[key] = tf.sigmoid(feature)
...
with tf.Session() as sess:
    ...
    while(True):
        count += 1 
        r_img, r_features,r_labels,r_probs = sess.run([images, feature_maps,labels,probs])
        print("image:",count)
        print("labels:")
        ...
        for i_pool ,key in enumerate(r_features):
            ...
            arg_probs= np.argsort(r_probs[i_pool])[0][::-1]
            sort_probs = np.sort(r_probs[i_pool])[0][::-1]
            keep_feature = []
